...
def load_krx_listing():
    """KRX 전체 상장 종목/ETF 목록을 가져온다."""
    try:
        df = fdr.StockListing('KRX')
        if df is None or df.empty:
            return pd.DataFrame(columns=['Code', 'Name'])

        # FinanceDataReader 버전에 따라 컬럼 구성이 다를 수 있으므로 방어적으로 처리
        ...
        out['Name'] = out['Name'].astype(str).str.replace(r'<[^>]+>', '', regex=True).str.strip()
        ...
        logging.warning(f"KRX 종목 목록 로딩 실패: {e}")
        return pd.DataFrame(columns=['Code', 'Name'])
