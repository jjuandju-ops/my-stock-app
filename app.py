...
def search_stock_list(query):
    ...
    # 2) 네이버 금융 자동완성 API (폴백)
    try:
        url = "https://ac.finance.naver.com/ac"
        params = {"q": query, "target": "stock", "mode": "json"}
        res = requests.get(url, params=params, headers=headers, timeout=5)
        res.raise_for_status()
        data = res.json()
...
def generate_v39_dashboard(query, code=None, name=None):
    if code is None or name is None:
        # query로부터 code, name 결정 (여기서는 이미 선택된 값이 전달됨)
